...
export function Card({
  ...
  footer, variant = "elevated", interactive = false, padding = "1.5rem", style, ...rest
}) {
  const variants = {
    elevated: { background: "var(--surface-card)", boxShadow: "var(--shadow-md)", border: "1px solid transparent" },
    outline:  { background: "var(--surface-card)", boxShadow: "none", border: "1px solid var(--border-subtle)" },
    soft:     { background: "var(--surface-raised)", boxShadow: "none", border: "1px solid var(--border-subtle)" },
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ padding, display: "flex", flexDirection: "column", gap: "0.6rem", flex: 1 }}>
        ...
        {meta && <div style={{ fontSize: "0.8125rem", color: "var(--text-muted)", fontFamily: "var(--font-sans)" }}>{meta}</div>}
        {children && <div style={{ fontSize: "0.9375rem", color: "var(--text-secondary)", lineHeight: 1.6, fontFamily: "var(--font-sans)" }}>{children}</div>}
