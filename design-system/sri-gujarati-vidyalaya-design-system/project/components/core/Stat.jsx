 ...
 */
export function Stat({ value, suffix = "", prefix = "", label, align = "center", tone = "default", animate = true, style, ...rest }) {
  ...
  React.useEffect(() => {
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((e) => { if (e.isIntersecting) { run(); cleanup(); } });
    }, { threshold: 0.35 });
    io.observe(node);
    window.addEventListener("scroll", onScroll, true);
    if (inView()) { run(); cleanup(); }

    return () => { window.removeEventListener("scroll", onScroll, true); io.disconnect(); cancelAnimationFrame(raf); };
  }, [value, animate]);

  const onDark = tone === "inverse";
  return (
    <div ref={ref} style={{ textAlign: align, ...style }} {...rest}>
      <div style={{
        fontFamily: "var(--font-display)", fontWeight: 500,
        fontSize: "clamp(2.25rem, 1.4rem + 2.6vw, 3.25rem)", lineHeight: 1,
        letterSpacing: "-0.02em",
        color: onDark ? "var(--gold-300)" : "var(--text-brand)",
        display: "flex", alignItems: "baseline", justifyContent: align === "center" ? "center" : "flex-start", gap: "0.05em",
      }}>
