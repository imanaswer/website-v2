...
(function () {
  ...
  const { Button, Badge, Stat, Avatar } = window.SriGujaratiVidyalayaDesignSystem_89aa2a;
            ...
            {streams.map((s, i) => (
              <Reveal key={s.t} delay={i * 100} style={{ height: "100%" }}>
                <div style={{ background: "var(--surface-card)", borderRadius: "var(--radius-xl)", padding: "2.2rem", height: "100%", boxShadow: "var(--shadow-md)", border: "1px solid var(--border-subtle)" }}>
                  <div style={{ width: 56, height: 56, borderRadius: "var(--radius-md)", background: "var(--maroon-700)", color: "var(--cream-50)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1.3rem" }}><Icon name={s.icon} size={30} /></div>
      ...
      <section style={{ background: "var(--maroon-900)", position: "relative", overflow: "hidden" }}>
        ...
        <div className="container container--wide" style={{ paddingBlock: "var(--space-16)", position: "relative" }}>
          <Reveal style={{ textAlign: "center", marginBottom: "var(--space-12)" }}>
            <Kicker tone="inverse"><span style={{ display: "inline-flex", margin: "0 auto" }}>Achievement dashboard</span></Kicker>
  ...
  function AcademicsPage({ onNavigate }) {
    return (
      <div>
