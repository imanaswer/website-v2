...
(function () {
  ...
  const { Button, Badge } = window.SriGujaratiVidyalayaDesignSystem_89aa2a;
      ...
      <section className="section">
        <div className="container container--narrow">
          <Reveal style={{ textAlign: "center", marginBottom: "var(--space-12)" }}>
            <Kicker><span style={{ display: "inline-flex", margin: "0 auto" }}>Our journey</span></Kicker>
  ...
  function Affiliation({ onNavigate }) {
    return (
        ...
        <div className="container container--wide" style={{ display: "flex", gap: "var(--space-12)", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          ...
          <Reveal delay={120} style={{ display: "flex", gap: "0.8rem", flexWrap: "wrap" }}>
            {["Kerala Govt. Recognised", "English Medium", "Co-Educational", "Higher Secondary", "Est. 1869"].map((b) => (
              <Badge key={b} tone="gold" style={{ background: "rgba(248,240,220,0.14)", color: "var(--gold-300)", fontSize: "0.82rem", padding: "0.5rem 0.9rem" }}>{b}</Badge>
            ))}
          </Reveal>
        </div>
      </section>
  ...
  function HeritagePage({ onNavigate }) {
    return (
      <div>
        ...
        <VMP />
        <VTimeline />
        <Affiliation onNavigate={onNavigate} />
      </div>
    );
  }
