...
(function () {
  ...
  const { Img, Icon, Reveal, Kicker, IMG } = window;
  const { Button, Badge, Stat, Card, Avatar } = window.SriGujaratiVidyalayaDesignSystem_89aa2a;
    ...
    return (
      <section style={{ position: "relative", minHeight: "min(92vh, 820px)", display: "flex", alignItems: "flex-end", overflow: "hidden" }}>
        <div className="hero-bg" style={{ position: "absolute", inset: 0 }}>
          <Img src={IMG.campus} alt="Sri Gujarati Vidyalaya campus"
            style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} />
        </div>
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(58,22,7,0.58) 0%, rgba(58,22,7,0.18) 32%, rgba(58,22,7,0.86) 100%)" }} />
        <div className="container container--wide" style={{ position: "relative", paddingBottom: "var(--space-12)", paddingTop: "var(--space-16)", width: "100%" }}>
          <Reveal>
            <Badge tone="gold" dot style={{ marginBottom: "1.4rem", background: "rgba(248,240,220,0.16)", color: "var(--gold-300)", backdropFilter: "blur(6px)" }}>Established 1869 · Kozhikode</Badge>
          </Reveal>
          <Reveal delay={80}>
      ...
      <section style={{ background: "var(--maroon-900)", position: "relative", overflow: "hidden" }}>
        ...
        <div className="container container--wide" style={{ paddingBlock: "var(--space-16)", position: "relative" }}>
          <Reveal style={{ textAlign: "center", marginBottom: "var(--space-12)" }}>
            <Kicker tone="inverse"><span style={{ display: "inline-flex", margin: "0 auto" }}>School at a glance</span></Kicker>
  ...
  window.HomeParts1 = { Hero, TrustBar, About, Glance, Academics, Facilities };
})();
