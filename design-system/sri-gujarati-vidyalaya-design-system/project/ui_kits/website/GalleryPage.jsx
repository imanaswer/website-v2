...
(function () {
  ...
  const { Badge } = window.SriGujaratiVidyalayaDesignSystem_89aa2a;
  ...
  function GalleryPage({ onNavigate }) {
    ...
    return (
      <div>
              ...
              {shown.map((p, i) => (
                <Reveal key={p.t + i} delay={(i % 6) * 60} style={{ breakInside: "avoid", marginBottom: "var(--space-5)" }}>
                  <button onClick={() => setBox(p)} style={{ display: "block", width: "100%", padding: 0, border: "none", cursor: "pointer", borderRadius: "var(--radius-lg)", overflow: "hidden", position: "relative", boxShadow: "var(--shadow-md)", background: "var(--surface-card)" }}>
                    ...
                    <span style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, transparent 55%, rgba(58,22,7,0.75))", display: "flex", alignItems: "flex-end", padding: "1rem", opacity: 0, transition: "opacity var(--dur)" }}
                      onMouseEnter={(e) => e.currentTarget.style.opacity = 1} onMouseLeave={(e) => e.currentTarget.style.opacity = 0}>
                      <span style={{ color: "var(--cream-50)", fontFamily: "var(--font-sans)", fontWeight: 600, fontSize: "0.9rem", display: "flex", justifyContent: "space-between", width: "100%", alignItems: "center" }}>{p.t}<Icon name="arrows-out-simple" size={16} /></span>
                    </span>
                  </button>
                </Reveal>
              ))}
        ...
        {box && (
          <div onClick={() => setBox(null)} style={{ position: "fixed", inset: 0, zIndex: 900, background: "rgba(22,15,9,0.88)", display: "flex", alignItems: "center", justifyContent: "center", padding: "var(--gutter)", backdropFilter: "blur(4px)" }}>
            <button onClick={() => setBox(null)} aria-label="Close" style={{ position: "absolute", top: 24, right: 24, width: 48, height: 48, borderRadius: "50%", border: "1px solid var(--border-on-dark)", background: "rgba(252,249,243,0.08)", color: "var(--cream-50)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="x" size={24} /></button>
            <figure onClick={(e) => e.stopPropagation()} style={{ margin: 0, maxWidth: 980, width: "100%" }}>
              ...
              <figcaption style={{ marginTop: "1rem", display: "flex", gap: "0.8rem", alignItems: "center", color: "var(--cream-50)" }}>
                <Badge tone="gold" style={{ background: "rgba(248,240,220,0.16)", color: "var(--gold-300)" }}>{box.cat}</Badge>
